...
def extract_terms(
    ...
    if not skip_acronyms:
        logging.info(
            f"Extracting acronyms from {text_file} using model {model}"
        )
    ...
    if not skip_definitions:
        logging.info(
            f"Extracting definitions from {text_file} using model {model}"
        )
    ...
    if not skip_relationships:
        logging.info(
            f"Extracting relationships from {text_file} using model {model}"
        )
