...
from utils import rename_file, write_text
...
def process_pdf(path_pdf, threshold_num_repetitions=5, **params):
    ...
    **params: Additional parameters to pass to the nougat extraction method.
    ...
    processed_output_nougat_file = p.parent / (
        p.stem + ".processed.nougat.txt"
    )
    processed_output_pymupdf_file = p.parent / (
        p.stem + ".processed.pymupdf.txt"
    )
    ...
    if not raw_output_nougat_file.exists():
        raise ValueError(
            f"File {raw_output_nougat_file} not found. Probably the nougat processing failed."
        )

    raw_text = raw_output_nougat_file.read_text()
    raw_text = remove_repeated_lines(
        raw_text, threshold_num_repetitions=threshold_num_repetitions
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batchsize", "-b", help="(Nougat) Batch size to use.", type=int
    )
    parser.add_argument("--model", "-m", help="(Nougat) model tag to use.")
    parser.add_argument(
        "--recompute",
        help="(Nougat) Recompute already computed PDF, discarding previous predictions.",
        action="store_true",
    )
    parser.add_argument(
        "--full-precision",
        help="(Nougat) Use float32 instead of bfloat16. Can speed up CPU conversion for some setups.",
        action="store_true",
    )
    parser.add_argument(
        "--markdown",
        help="(Nougat) Add postprocessing step for markdown compatibility (default).",
        action="store_true",
    )
    parser.add_argument(
        "--no-skipping",
        help="(Nougat) Don't apply failure detection heuristic.",
        action="store_true",
    )
    parser.add_argument(
        "--pages",
        "-p",
        help="(Nougat) Provide page numbers like '1-4,7' for pages 1 through 4 and page 7. Only works for single PDFs.",
    )
    parser.add_argument(
        "--threshold_num_repetitions",
        help="(Nougat) Threshold number of repetitions to remove repeated lines.",
        type=int,
        default=5,
    )
