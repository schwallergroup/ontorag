...
def analyze_ontology(ontology_path):
    ...
    single_child_classes = [
        cls for cls, children in class_children.items() if len(children) == 1
    ]
    above_avg_children_classes = [
        cls
        for cls, children in class_children.items()
        if len(children) > avg_children
    ]
...
def main():
    ontology_path = (
        "/home/matt/Proj/Hermetica/Ontology/Ontologies/PolymerOntology.owl"
    )
    ...
    print(
        f"Classes with a Single Child: {len(results['single_child_classes'])}"
    )
    print(
        f"Classes with More than Average Children: {len(results['above_avg_children_classes'])}"
    )
